...
if __name__ == "__main__":
    ...
    emptylabel = tk.Label(root, text="  ").grid(
        row=12, column=0, columnspan=9, sticky="nsew")
    # emptylabel = tk.Label(root, text="       ").grid(row=0, column=1, sticky="nsew")
    emptylabel = tk.Label(root, text="       ").grid(
        row=3, column=7, sticky="nsew")
    emptylabel = tk.Label(root, text="       ").grid(
        row=6, column=7, sticky="nsew")
    emptylabel = tk.Label(root, text="       ").grid(
        row=11, column=1, sticky="nsew")
    emptylabel = tk.Label(root, text="  ").grid(
        row=24, column=0, sticky="nsew")
    emptylabel = tk.Label(root, text="              ").grid(
        row=25, column=1, columnspan=9, sticky="nsew")
    emptylabel = tk.Label(root, text="       ").grid(
        row=20, column=1, columnspan=4, sticky="nsew")
    emptylabel = tk.Label(root, text="       ").grid(
        row=29, column=1, columnspan=4, sticky="nsew")
    emptylabel = tk.Label(root, text="       ").grid(
        row=33, column=1, columnspan=4, sticky="nsew")
    ...
    label5 = tk.Label(root, text="   Input parameters",
    ...
    # Dataset
    label_dataset = tk.Label(root, text="   Dataset name:").grid(
        row=13, columnspan=2, sticky="w")
    ...
    labelp = tk.Label(root, text=" p-value:").grid(row=15,
                                                   column=1, sticky="w")
    label_p_info = tk.Label(
        root, text='   % of image width that is a black/white strip. See Script_02_AutomaticFiducialDetection', font=('calibre', 7, 'italic'))
    ...
    label_stripes = tk.Label(root, text="    Stripes location:").grid(
        row=17, columnspan=2, sticky="w")
    ...
    def resolution_list(camera):
        ...
        resolution_file = r"{}/camera/{}_Airphoto_Photo_dimensions_vs_dpi.csv".format(
            path, camera)
        ...
        for choice in res_list:
            entry_input_res['menu'].add_command(
                label=choice, command=tk._setit(chosen_input_res, choice))
            entry_ouput_res['menu'].add_command(
                label=choice, command=tk._setit(chosen_output_res, choice))
    ...
    labelcamera = tk.Label(root, text=" Camera system:").grid(
        row=21, column=1, sticky="w")
    chosen_camera = tk.StringVar(root)
    chosen_camera.set("Choose a camera")  # by default
    entry_camera = tk.OptionMenu(
        root, chosen_camera, *camera_value_list, command=resolution_list)
    ...
    label_SharpIntensity = tk.Label(root, text=" Sharpening intensity:").grid(
        row=25, column=2, sticky="w")
    chosen_SharpIntensity = tk.StringVar(root)
    chosen_SharpIntensity.set(SharpIntensity_value_list[2])  # by default
    entry_SharpIntensity = tk.OptionMenu(
        root, chosen_SharpIntensity, *SharpIntensity_value_list)
    ...
    c = ttk.Checkbutton(root, text="Script_01: Canvas Sizing",
                        variable=check_01).grid(row=31, column=1, sticky="w")
    c = ttk.Checkbutton(root, text="Script_02: Fiducial Detection",
                        variable=check_02).grid(row=31, column=2, sticky="w")
    c = ttk.Checkbutton(root, text="Script_03: Reproject",
                        variable=check_03).grid(row=32, column=1, sticky="w")
    c = ttk.Checkbutton(root, text="Script_04: Downsampling",
                        variable=check_04).grid(row=32, column=2, sticky="w")

    entry_input_folder = tk.Button(root, text="Select folder", command=lambda: find_input_folder(
        entry_input_images, "Select aerial image folder")).grid(row=3, column=9, sticky="w")
    entry_output_folder = tk.Button(root, text="Select folder", command=lambda: find_output_folder(
        entry_output_images, "Select output directory")).grid(row=6, column=9, sticky="w")
    entry_template_folder = tk.Button(root, text="Select folder", command=lambda: find_template_folder(
        entry_fidu, "Select template directory")).grid(row=10, column=9, sticky="w")
    ...
    main_script = partial(main_script, input_folder, output_folder, template_folder,
    ...
    buttonRun = ttk.Button(root, text="Run", style='Accent.TButton', command=main_script).grid(
        row=34, column=7, columnspan=6, sticky="nsew")
